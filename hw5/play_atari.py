...
parser.add_argument("--compress-replay", action='store_true', default=False, help="if set replay memory will be compressed with blosc, allowing much larger replay capacity")
...
State.setup(args)
print "aaaaa", State.useCompression
...
def runEpoch(minEpochSteps, evalWithEpsilon=None):
	...
	while environment.getStepNumber() - stepStart < minEpochSteps:
		...
		maxStep = 5000 # only 5000 step
		step_start = environment.getStepNumber()
		
		while (not environment.isGameOver()) and (environment.getStepNumber() - step_start < maxStep):
